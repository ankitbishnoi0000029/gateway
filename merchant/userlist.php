...
if (isset($_POST['delete'])) {
    ...
    if ($rpt) {
    $fetchuser = $conn->query("SELECT user_token FROM `users` WHERE mobile='$mb'")->fetch_assoc();
    $deluser_token = $fetchuser["user_token"];
    // inactive other merchant
$tablesarr = ["hdfc","bharatpe_tokens","freecharge","googlepay_tokens","merchant","paytm_tokens","phonepe_tokens"];

foreach($tablesarr as $tables){
    $fetchmerchant = $conn->query("SELECT user_token FROM `$tables` WHERE user_token = '$deluser_token' AND status = 'Active'");
    if($fetchmerchant->num_rows > 0){
        $conn->query("DELETE FROM `$tables` WHERE user_token = '$deluser_token'");
    }
}
...
<main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-users"></i> Manage User</h1>
...
echo "<td>" . htmlspecialchars($row['expiry'], ENT_QUOTES, 'UTF-8') . "</td>";
    ...
    <div class="col-12">
        <form action="edituser.php?mobileno=<?php echo $row['mobile']; ?>" method="post">
            <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
            <input type="hidden" name="mobileno" value="<?php echo $row['mobile']; ?>">

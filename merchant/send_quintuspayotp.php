...
if(isset($_POST['Verify'])){ //from last page
   ...
   $mobileNumber = filter_var($_REQUEST['quintus_mobile'], FILTER_VALIDATE_INT);
   
   
    // The URL to send the POST request to
    $url = "https://miniapi.shop/api/quintustech/send_otp.php?mobile=$mobileNumber";
    ...
    curl_setopt($ch, CURLOPT_GET, true); // Set method to POST
    ...
    if ($status1 == 200) {
        ...
        echo '<script src="js/jquery-3.2.1.min.js"></script>';echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11.0.18"></script>';echo '<script>$("#loading_ajax").hide();
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        title: "Quintus Pay UPI Settings",
                        html: `
                            <form id="hdfcForm" method="POST" action="quintuspay_verify" class="mb-2">
         ...
         $errorMessage=$responseArray['response']['message'];
